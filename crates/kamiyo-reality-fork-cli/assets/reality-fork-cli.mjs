...
var RealityForkStudioClient = class {
  ...
  async requestJson(path22, init) {
    const response = await this.fetchImpl(`${this.baseUrl}${path22}`, init);
    if (!response.ok) {
      throw new Error(`Reality Fork request failed (${response.status} ${path22})`);
    }
...
function configHome(dirOverride) {
  return dirOverride || process.env.XDG_CONFIG_HOME || path2.join(os2.homedir(), ".config");
}
...
var ConfigStore = class _ConfigStore {
  ...
  static load(dirOverride) {
    const dirPath = path2.join(configHome(dirOverride), "kamiyo", "reality-fork-cli");
    ensurePrivateDir(dirPath);
    const filePath = path2.join(dirPath, "config.json");
  ...
  get historyPath() {
    return path2.join(this.dirPath, "history");
  }
  get sessionLogPath() {
    const configured = this.config.sessionLog.path;
    if (configured) {
      ensurePrivateDir(path2.dirname(configured));
      return configured;
    }
    return path2.join(this.dirPath, "sessions.jsonl");
  }
  save() {
    ensurePrivateDir(this.dirPath);
    fs2.writeFileSync(this.filePath, JSON.stringify(this.config, null, 2), "utf8");
...
var SessionLogger = class {
  ...
  append(entry) {
    if (!this.enabled)
      return;
    fs3.mkdirSync(requireParent(this.filePath), { recursive: true, mode: 448 });
    fs3.appendFileSync(this.filePath, `${JSON.stringify(entry)}
`, "utf8");
    try {
...
function shellQuote(value) {
  if (!value)
    return "''";
  if ([...value].every((ch) => /[A-Za-z0-9\-_.:\/]/.test(ch))) {
    return value;
  }
...
function sessionLogCheck(store) {
  ...
  try {
    fs4.mkdirSync(path3.dirname(filePath), { recursive: true, mode: 448 });
    if (!fs4.existsSync(filePath)) {
      fs4.writeFileSync(filePath, "", "utf8");
    }
...
function buildProgram(state) {
  ...
  program2.name(rootProgramName()).description("Focused CLI for Reality Fork fixtures and remote project operations").option("--api-url <url>", "Reality Fork API base URL").option("--profile <name>", "Profile name").addOption(jsonOption).option("--quiet", "Suppress non-essential output").option("--verbose", "Show debug output");
  ...
  uploads.command("add").argument("<paths...>", "File paths").description("Upload files and return upload ids").action(wrapAction(state, "uploads add", async ({ effective }, [pathsArg]) => {
    ...
    for (const filePath of paths) {
      const absolutePath = path3.resolve(String(filePath));
      const bytes = fs4.readFileSync(absolutePath);
      formData.append("files", new Blob([bytes], { type: guessMimeType(absolutePath) }), path3.basename(absolutePath));
    }
  ...
  projects.command("create").description("Create a new project on a remote Reality Fork API").requiredOption("--prompt <prompt>", "Prompt or claim to evaluate").option("--title <title>", "Project title").option("--description <text>", "Project description").option("--tag <tag>", "Attach a tag", (value, acc) => {
    ...
    if (commandOptions.file.length > 0) {
      ...
      for (const filePath of commandOptions.file) {
        ...
        formData.append("files", new Blob([fs4.readFileSync(absolutePath)], { type: guessMimeType(absolutePath) }), path3.basename(absolutePath));
      }
  ...
  program2.command("shell").description("Start the interactive shell").action(wrapAction(state, "shell", async ({ store, effective, runNested }) => {
    ...
    const existingHistory = fs4.existsSync(historyPath) ? fs4.readFileSync(historyPath, "utf8").split("\n").filter(Boolean) : [];
    const internal = rl;
    ...
    try {
      ...
      fs4.mkdirSync(path3.dirname(historyPath), { recursive: true, mode: 448 });
      fs4.writeFileSync(historyPath, nextHistory.join("\n") + (nextHistory.length > 0 ? "\n" : ""), "utf8");
    }
  }));
  return program2;
}
