/*
 * dark_forest-core: Verification benchmarks
 */
...
static void create_random_proof(dark_forest_proof_t *proof) {
...
static void bench_batch_verify(bench_result_t *r, size_t batch_size) {
    dark_forest_ctx_t *ctx = dark_forest_ctx_create(NULL);
    dark_forest_proof_t *proofs = malloc(batch_size * sizeof(dark_forest_proof_t));
    ...
    for (int i = 0; i < WARMUP_ITERS / 10; i++) {
        dark_forest_batch_t *batch = dark_forest_batch_create(ctx, batch_size);
        for (size_t j = 0; j < batch_size; j++) {
            dark_forest_batch_add(batch, &proofs[j]);
        }
...
static void bench_proof_create(bench_result_t *r) {
    ...
    /* Warmup */
    for (int i = 0; i < WARMUP_ITERS; i++) {
        dark_forest_proof_create(&proof, DARK_FOREST_PROOF_REPUTATION, 50, agent_pk, commitment, proof_data, 256);
    }
...
static void bench_nullifier(bench_result_t *r) {
    ...
    /* Warmup */
    for (int i = 0; i < WARMUP_ITERS; i++) {
        dark_forest_compute_nullifier(out, agent_pk, i);
    }
...
int main(void) {
    ...
    printf("\n");
    printf("+-----------------------------------------------------------+\n");
    printf("|         dark_forest-core Verification Benchmark                |\n");
    printf("+-----------------------------------------------------------+\n");
