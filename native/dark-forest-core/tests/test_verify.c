/*
 * dark_forest-core: Verification engine tests
 */

#include "../src/dark_forest.h"
...
static void test_ctx_create_destroy(void) {
    dark_forest_ctx_t *ctx = dark_forest_ctx_create(NULL);
    assert(ctx != NULL);
    dark_forest_ctx_destroy(ctx);
}
...
static void test_ctx_with_config(void) {
    ...
    dark_forest_ctx_t *ctx = dark_forest_ctx_create(&config);
    assert(ctx != NULL);
    dark_forest_ctx_destroy(ctx);
}
...
static void test_batch_create(void) {
    ...
    dark_forest_batch_t *batch = dark_forest_batch_create(ctx, 256);
    assert(batch != NULL);

    dark_forest_batch_destroy(batch);
    dark_forest_ctx_destroy(ctx);
}
...
static void test_proof_create(void) {
    ...
    dark_forest_result_t r = dark_forest_proof_create(
        &proof,
        DARK_FOREST_PROOF_REPUTATION,
        75,
        agent_pk,
        commitment,
        NULL,
    ...
    assert(r == DARK_FOREST_OK); (void)r;
    assert(proof.type == DARK_FOREST_PROOF_REPUTATION);
...
static void test_proof_malformed(void) {
    ...
    dark_forest_result_t r = dark_forest_verify(ctx, &proof);
    assert(r == DARK_FOREST_ERR_MALFORMED); (void)r;
...
static void test_threshold_check(void) {
     ...
     */
    dark_forest_proof_create(&proof, DARK_FOREST_PROOF_REPUTATION, 30, agent_pk, commitment, NULL, 0);

    dark_forest_result_t r = dark_forest_verify(ctx, &proof);
    /* Proof rejected - either malformed (invalid points) or below threshold */
    assert(r == DARK_FOREST_ERR_BELOW_THRESHOLD || r == DARK_FOREST_ERR_MALFORMED); (void)r;
...
static void test_nullifier_computation(void) {
    ...
    dark_forest_compute_nullifier(nullifier1, agent_pk, 0);
    dark_forest_compute_nullifier(nullifier2, agent_pk, 1);
...
static void test_nullifier_deterministic(void) {
    ...
    dark_forest_compute_nullifier(nullifier1, agent_pk, 12345);
    dark_forest_compute_nullifier(nullifier2, agent_pk, 12345);
...
static void test_batch_empty(void) {
    ...
    dark_forest_result_t r = dark_forest_batch_verify(batch);
    assert(r == DARK_FOREST_OK); (void)r;
...
static void test_batch_add_verify(void) {
    ...
    dark_forest_proof_t proof;
    dark_forest_proof_create(&proof, DARK_FOREST_PROOF_REPUTATION, 80, agent_pk, commitment, proof_data, 256);

    dark_forest_result_t r = dark_forest_batch_add(batch, &proof);
    assert(r == DARK_FOREST_OK); (void)r;

    r = dark_forest_batch_verify(batch);
    assert(r == DARK_FOREST_OK);
...
static void test_stats(void) {
    ...
    dark_forest_get_stats(ctx, &stats);
...
static void test_poseidon_consistency(void) {
    ...
    dark_forest_compute_nullifier(out1, pk, 0);
    dark_forest_compute_nullifier(out2, pk, 0);
...
static void test_poseidon_circomlib_vector(void) {
    ...
    uint8_t result[32];
    dark_forest_compute_nullifier(result, pk, 2);
...
int main(void) {
    printf("\n");
    printf("dark_forest-core: Verification Engine Tests\n");
    printf("========================================================\n\n");
