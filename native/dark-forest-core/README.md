...
High-performance native proof verification engine for DARK_FOREST × KAMIYO integration.
...
for (int i = 0; i < n; i++) {
    dark_forest_batch_add(batch, &proofs[i]);
}
dark_forest_batch_verify(batch);

dark_forest_result_t results[256];
size_t count;
dark_forest_batch_get_results(batch, results, &count);
