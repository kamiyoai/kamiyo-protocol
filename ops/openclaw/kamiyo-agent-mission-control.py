...
KAMIYO_AGENT_RUNTIME_PATH = STATE_DIR / 'kamiyo-agent-runtime.json'
...
def run() -> int:
    ...
    kamiyo_agent_runtime = read_json(KAMIYO_AGENT_RUNTIME_PATH, {'ok': False})
    if not isinstance(kamiyo_agent_runtime, dict):
        kamiyo_agent_runtime = {'ok': False}
    runtime_summary = kamiyo_agent_runtime.get('summary') if isinstance(kamiyo_agent_runtime.get('summary'), dict) else {}
    ...
    runtime_ok = bool(kamiyo_agent_runtime.get('ok'))
        ...
        backlog.append(
            {
                ...
                'priority': 'high',
                'title': 'Recover Kamiyo Agent runtime health',
