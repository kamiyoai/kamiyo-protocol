...
def request_json(url: str, headers: dict[str, str] | None = None) -> Any:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            'User-Agent': 'kyoshin-trading-feed/3.0',
            **(headers or {}),
        },
        method='GET',
    )
...
def load_singularity_payload() -> Any:
    ...
    for record in markets:
        ...
        if market_id in positions_by_market:
            record['_kyoshin_position'] = positions_by_market[market_id]
...
def normalize_candidate(venue: str, record: dict[str, Any], index: int) -> dict[str, Any] | None:
    ...
    if liquidity <= 0:
        liquidity = orderbook_depth_usd(record.get('_kyoshin_orderbook'))
        ...
        metadata.update(
            {
                ...
                'singularityPositionHint': compact_record(
                    record.get('_kyoshin_position')
                    if isinstance(record.get('_kyoshin_position'), dict)
                    else {}
                ),
            }
        )
...
def run() -> int:
    ...
    feed_payload = {
        ...
        'at': now_iso(),
        'startedAt': started_at,
        'source': 'kyoshin_trading_feed_v3',
