...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-creator-fee-inflow-route.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_creator_fee_inflow_route', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-creator-fee-inflow-route.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinCreatorFeeInflowRouteTests(unittest.TestCase):
    def setUp(self):
