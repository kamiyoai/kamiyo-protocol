...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-revenue-guard.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_revenue_guard', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-revenue-guard.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentRevenueGuardTests(unittest.TestCase):
    def setUp(self):
