...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-mission-control.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_mission_control', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-mission-control.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentMissionControlTests(unittest.TestCase):
    def setUp(self):
        ...
        self.mod.KAMIYO_AGENT_RUNTIME_PATH = self.state_dir / 'kamiyo-agent-runtime.json'
    ...
    def test_includes_sentry_items_in_backlog(self):
        ...
        self._write_json(self.mod.KAMIYO_AGENT_RUNTIME_PATH, {'ok': True, 'summary': {'lastTickStatus': 'ok', 'mode': 'execute'}})
        self._write_json(
