...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-autonomy-loop.sh'
...
class KamiyoAgentAutonomyLoopContractTests(unittest.TestCase):
    def setUp(self):
    ...
    def _write_default_scripts(self) -> None:
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-x402-feed.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-dx-terminal-feed.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-sync-feed-config.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-context-guard.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-sentry-pipeline.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-tool-health.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-marketplace-intake.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-receipt-sync.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-swarm-governor.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-swarm-planner.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-runtime-bridge.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-mission-control.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-revenue-guard.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-trading-feed.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-trading-exec.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-trading-staking-route.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-x402-agentcash.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-clawmart-staking-route.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-clawmart-monitor.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-creator-fee-inflow-route.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-distribution-engine.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-artifact-contracts.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-learnings.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-memory-extract.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
        ...
        self._write_exec(
            self.bin_dir / 'kamiyo-agent-operator-log.py',
            f"""#!/usr/bin/env bash
set -euo pipefail
...
""",
    ...
    def test_runtime_bridge_soft_gate_when_disabled(self):
        ...
        result = self._run_loop({'KYO_REQUIRE_KAMIYO_AGENT_RUNTIME': 'false'})
        self.assertEqual(result.returncode, 0, msg=result.stderr)
    ...
    def test_missing_artifact_contracts_script_degrades_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-artifact-contracts.py').unlink()
    ...
    def test_missing_receipt_sync_script_degrades_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-receipt-sync.py').unlink()
...
""",
    ...
    def test_sentry_pipeline_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-sentry-pipeline.py').unlink()
    ...
    def test_revenue_guard_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-revenue-guard.py').unlink()
    ...
    def test_clawmart_staking_route_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-clawmart-staking-route.py').unlink()
    ...
    def test_creator_fee_inflow_route_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-creator-fee-inflow-route.py').unlink()
    ...
    def test_clawmart_monitor_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-clawmart-monitor.py').unlink()
    ...
    def test_memory_extract_is_hard_gate_when_required(self):
        self._write_default_scripts()
        (self.bin_dir / 'kamiyo-agent-memory-extract.py').unlink()
        current_hour = datetime.now(timezone.utc).strftime('%H')
