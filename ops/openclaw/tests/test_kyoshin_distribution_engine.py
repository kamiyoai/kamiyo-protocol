...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-distribution-engine.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_distribution_engine', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-distribution-engine.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinDistributionEngineTests(unittest.TestCase):
    def setUp(self):
            ...
            [
                {
                    ...
                    'posts': [{'id': 'post-1', 'channel': 'x', 'text': 'Kyoshin offer is live'}],
                }
            ]
        )
