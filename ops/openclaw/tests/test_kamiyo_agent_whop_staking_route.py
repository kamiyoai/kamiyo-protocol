...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-whop-staking-route.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_whop_staking_route', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-whop-staking-route.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentWhopStakingRouteTests(unittest.TestCase):
    def setUp(self):
