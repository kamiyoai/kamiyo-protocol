...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-operator-log.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_operator_log', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-operator-log.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentOperatorLogTests(unittest.TestCase):
    def setUp(self):
    ...
    def _run(self, argv: list[str] | None = None) -> tuple[int, dict]:
        args = argv or ['kamiyo-agent-operator-log.py']
        stdout = io.StringIO()
    ...
    def test_writes_daily_summary_and_metrics(self):
        ...
        code, summary = self._run(['kamiyo-agent-operator-log.py', '--status', 'ok', '--cycle', '42'])
    ...
    def test_does_not_publish_twice_same_day_unless_forced(self):
        ...
        code1, summary1 = self._run(['kamiyo-agent-operator-log.py', '--status', 'ok', '--cycle', '1'])
        self.assertEqual(code1, 0)
        self.assertEqual(summary1.get('published'), True)

        code2, summary2 = self._run(['kamiyo-agent-operator-log.py', '--status', 'ok', '--cycle', '2'])
        self.assertEqual(code2, 0)
        self.assertEqual(summary2.get('published'), False)

        code3, summary3 = self._run(['kamiyo-agent-operator-log.py', '--status', 'ok', '--cycle', '3', '--force'])
    ...
    def test_carries_blockers_from_guard_and_monitor(self):
        ...
        code, summary = self._run(['kamiyo-agent-operator-log.py', '--status', 'degraded', '--cycle', '8'])
        self.assertEqual(code, 0)
