...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-whop-monitor.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_whop_monitor', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-whop-monitor.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinWhopMonitorTests(unittest.TestCase):
    def setUp(self):
