...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-dx-terminal-feed.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_dx_terminal_feed', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-dx-terminal-feed.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentDxTerminalFeedTests(unittest.TestCase):
    def setUp(self):
