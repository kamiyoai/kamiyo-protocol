...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-clawmart-staking-route.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_clawmart_staking_route', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-clawmart-staking-route.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinClawMartStakingRouteTests(unittest.TestCase):
    def setUp(self):
