...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-artifact-contracts.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_artifact_contracts', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-artifact-contracts.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentArtifactContractsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.prev_require_runtime = os.environ.get('KYO_REQUIRE_KAMIYO_AGENT_RUNTIME')
    ...
    def tearDown(self):
        if self.prev_require_runtime is None:
            os.environ.pop('KYO_REQUIRE_KAMIYO_AGENT_RUNTIME', None)
        else:
            os.environ['KYO_REQUIRE_KAMIYO_AGENT_RUNTIME'] = self.prev_require_runtime
    ...
    def _write_valid_artifacts(self, include_runtime: bool = True):
        ...
        if include_runtime:
            self._write_json(
                self.mod.STATE_DIR / 'kamiyo-agent-runtime.json',
    ...
    def test_run_passes_with_valid_artifacts(self):
        os.environ['KYO_REQUIRE_KAMIYO_AGENT_RUNTIME'] = 'true'
    ...
    def test_runtime_state_is_optional_when_runtime_requirement_disabled(self):
        os.environ['KYO_REQUIRE_KAMIYO_AGENT_RUNTIME'] = 'false'
