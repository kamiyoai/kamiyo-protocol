...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-memory-extract.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_memory_extract', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-memory-extract.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinMemoryExtractTests(unittest.TestCase):
    def setUp(self):
    ...
    def _run(self, date_value: str) -> dict:
        stdout = io.StringIO()
        with patch('sys.argv', ['kyoshin-memory-extract.py', '--date', date_value]):
