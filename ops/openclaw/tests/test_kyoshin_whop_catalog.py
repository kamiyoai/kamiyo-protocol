...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-whop-catalog.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_whop_catalog', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-whop-catalog.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinWhopCatalogTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_upserts_catalog_and_writes_mapping(self):
        ...
        with patch.object(self.mod, 'list_products', return_value=[]), patch.object(
            ...
            return_value=({'id': 'prod_1', 'slug': 'kyoshin-audit', 'name': 'Kyoshin 24h Revenue Audit'}, 'created'),
        ), patch.object(self.mod, 'list_plans', return_value=[]), patch.object(
            self.mod,
            'create_or_update_plan',
            return_value=({'id': 'plan_1', 'slug': 'kyoshin-audit-one-time'}, 'created'),
        ):
            code, summary = self._run()
