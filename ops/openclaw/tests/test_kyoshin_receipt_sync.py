...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-receipt-sync.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_receipt_sync', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-receipt-sync.py')
...
class KyoshinReceiptSyncTests(unittest.TestCase):
    def setUp(self):
        ...
        self.mod.STATE_PATH = self.mod.STATE_DIR / 'kyoshin-receipt-sync-state.json'
        self.mod.LOG_PATH = self.mod.LOG_DIR / 'kyoshin-receipt-sync.jsonl'
    ...
    def test_sync_exports_success_and_failed_rows(self):
        ...
        with patch.dict(
            os.environ,
            {
                'KYO_KYOSHIN_DB_PATH': str(self.db_path),
    ...
    def test_cursor_prevents_duplicate_rows(self):
        ...
        with patch.dict(os.environ, {'KYO_KYOSHIN_DB_PATH': str(self.db_path)}, clear=False):
            first = self.mod.run()
            second = self.mod.run()
