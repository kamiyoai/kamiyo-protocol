...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-sync-feed-config.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_sync_feed_config', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-sync-feed-config.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KyoshinSyncFeedConfigTests(unittest.TestCase):
    def setUp(self):
