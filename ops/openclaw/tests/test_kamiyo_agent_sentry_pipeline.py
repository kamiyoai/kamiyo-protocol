...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kamiyo-agent-sentry-pipeline.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kamiyo_agent_sentry_pipeline', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kamiyo-agent-sentry-pipeline.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class KamiyoAgentSentryPipelineTests(unittest.TestCase):
    def setUp(self):
        ...
        self.mod.LOG_PATH = self.logs_dir / 'kamiyo-agent-sentry-pipeline.jsonl'
    ...
    def _run(self, argv: list[str]) -> tuple[int, dict]:
        stdout = io.StringIO()
        with patch('sys.argv', ['kamiyo-agent-sentry-pipeline.py'] + argv):
