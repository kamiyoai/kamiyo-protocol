...
SCRIPT_PATH = Path(__file__).resolve().parent.parent / 'kyoshin-x402-agentcash.py'


def load_module():
    spec = importlib.util.spec_from_file_location('kyoshin_x402_agentcash', SCRIPT_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError('failed to load kyoshin-x402-agentcash.py')
...
class KyoshinX402AgentCashTests(unittest.TestCase):
    def setUp(self):
