...
OUTPUT_PATH = STATE_DIR / 'kyoshin-runtime.json'
METRICS_OUTPUT_PATH = STATE_DIR / 'kyoshin-runtime-metrics.prom'
LOG_PATH = LOG_DIR / 'kyoshin-runtime-bridge.jsonl'
...
HEALTH_URL = os.getenv('KYO_KYOSHIN_RUNTIME_HEALTH_URL', 'http://127.0.0.1:4020/health').strip()
STATUS_URL = os.getenv('KYO_KYOSHIN_RUNTIME_STATUS_URL', 'http://127.0.0.1:4020/status').strip()
METRICS_URL = os.getenv('KYO_KYOSHIN_RUNTIME_METRICS_URL', 'http://127.0.0.1:4020/metrics').strip()
TOKEN = os.getenv('KYO_KYOSHIN_RUNTIME_TOKEN', '').strip()
...
def request_json(url: str) -> Any:
    ...
    headers = {'accept': 'application/json', 'user-agent': 'kyoshin-runtime-bridge/1.0'}
    if TOKEN:
        headers['authorization'] = f'Bearer {TOKEN}'
        headers['x-kyoshin-token'] = TOKEN
...
def request_text(url: str) -> str:
    if not is_supported_url(url):
        raise ValueError(f'unsupported_url:{url}')
    headers = {'accept': 'text/plain', 'user-agent': 'kyoshin-runtime-bridge/1.0'}
    if TOKEN:
        headers['authorization'] = f'Bearer {TOKEN}'
        headers['x-kyoshin-token'] = TOKEN
    request = urllib.request.Request(url, headers=headers, method='GET')
...
def run() -> int:
    ...
    append_log({'at': failed['at'], 'event': 'kyoshin_runtime_bridge', 'ok': False, 'error': error})
