...
def fetch_json(path: str) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            'User-Agent': 'kamiyo-agent-clawmart-monitor/1.0',
        },
        method='GET',
    )
...
def staking_route_checkpoint(receipts_path: Path, required_pool_url: str) -> dict[str, Any]:
    ...
    for raw in rows:
        ...
        if source and source not in {'clawmart', 'claw_mart', 'kamiyo_agent_clawmart'}:
            continue
    ...
    return [
        {
            ...
            'channel': 'x',
            'text': (
                'Kamiyo Agent launch pricing is live for autonomous ops: '
        ...
        },
        {
            ...
            'text': (
                ...
                f'Kamiyo Agent x402 Facilitator Pipeline ($99) {x402_url}. '
...
def build_outreach_targets(listings: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    base_dm = (
        'Hey {target}, quick note: we packaged a Kamiyo Agent operator offer for {pain}. '
