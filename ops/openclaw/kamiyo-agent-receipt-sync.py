...
STATE_PATH = STATE_DIR / 'kamiyo-agent-receipt-sync-state.json'
LOG_PATH = LOG_DIR / 'kamiyo-agent-receipt-sync.jsonl'
...
def parse_db_path() -> Optional[Path]:
    configured = os.getenv('KYO_KAMIYO_AGENT_DB_PATH', '').strip()
    if configured:
    ...
    fallback_candidates = [
        ...
        Path.cwd() / 'services' / 'kamiyo-agent' / 'output' / 'kamiyo-agent' / 'state.db',
        Path.cwd() / 'output' / 'kamiyo-agent' / 'state.db',
        HOME_DIR / '.openclaw' / 'workspace' / 'runtime' / 'kamiyo-agent' / 'state.db',
    ]
...
def run() -> int:
    ...
    try:
        ...
        append_json_line(
            ...
            {
                'at': now_iso(),
                'event': 'kamiyo_agent_receipt_sync',
                'ok': True,
                'syncedRows': len(rows),
                'appendedReceipts': appended,
                'lastRowId': new_cursor,
