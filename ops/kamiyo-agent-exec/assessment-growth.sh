...
if [[ -z "${KYO_ASSESS_STATE_PATH:-}" ]]; then
  state_dir="output/kamiyo-agent-exec"
...
cd "$APP_ROOT"
if [[ "$MODE" == "auto" ]]; then
  exec corepack pnpm exec tsx scripts/kamiyo-agent-assessment-growth.ts
fi

exec env KYO_ASSESS_MODE="$MODE" corepack pnpm exec tsx scripts/kamiyo-agent-assessment-growth.ts
