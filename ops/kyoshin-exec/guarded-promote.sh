...
PROMOTE_BIN="${PROMOTE_BIN:-/usr/local/bin/kamiyo-kyoshin-exec-stage}"
PREFLIGHT_BIN="${PREFLIGHT_BIN:-/usr/local/bin/kamiyo-kyoshin-exec-preflight}"
...
evaluate_gates() {
  ...
  host="$(get_kv KYOSHIN_HTTP_HOST)"
  port="$(get_kv KYOSHIN_HTTP_PORT)"
