...
export interface Config {
  ...
  FAIRSCALE_TRUST_EVENTS_URL: string;
  FAIRSCALE_TRUST_EVENTS_KEY: string;
  FAIRSCALE_TRUST_EVENTS_KEY_HEADER: string;
  FAIRSCALE_TRUST_EVENTS_SIGNATURE_MODE: 'disabled' | 'hmac-sha256-body';
  FAIRSCALE_TRUST_EVENTS_SIGNATURE_HEADER: string;
  FAIRSCALE_TRUST_EVENTS_HMAC_SECRET: string;
  FAIRSCALE_TRUST_EVENTS_EVENT_SIGNATURE_FIELD: string;
  FAIRSCALE_TRUST_EVENTS_BATCH_SIZE: number;
  FAIRSCALE_TRUST_EVENTS_FLUSH_INTERVAL_MS: number;
...
const DEFAULTS: Partial<Config> = {
  ...
  FAIRSCALE_TRUST_EVENTS_FLUSH_INTERVAL_MS: 5_000,
...
export function validateConfig(): ValidationResult {
  ...
  const fairscaleTrustEventsUrl = process.env.FAIRSCALE_TRUST_EVENTS_URL;
  if (fairscaleTrustEventsUrl) {
    try {
      new URL(fairscaleTrustEventsUrl);
    } catch {
      errors.push('FAIRSCALE_TRUST_EVENTS_URL must be a valid URL');
    }
  }

  const fairscaleSignatureMode =
    process.env.FAIRSCALE_TRUST_EVENTS_SIGNATURE_MODE ||
    DEFAULTS.FAIRSCALE_TRUST_EVENTS_SIGNATURE_MODE!;
  if (!['disabled', 'hmac-sha256-body'].includes(fairscaleSignatureMode)) {
    errors.push('FAIRSCALE_TRUST_EVENTS_SIGNATURE_MODE must be disabled or hmac-sha256-body');
  }
  ...
  if (fairscaleTrustEventsUrl && !process.env.FAIRSCALE_TRUST_EVENTS_KEY?.trim()) {
    warnings.push('FAIRSCALE_TRUST_EVENTS_URL set but FAIRSCALE_TRUST_EVENTS_KEY is empty');
  }
  if (
    fairscaleTrustEventsUrl &&
...
export function getConfig(): Config {
  ...
  cachedConfig = {
    ...
    FAIRSCALE_TRUST_EVENTS_URL:
      process.env.FAIRSCALE_TRUST_EVENTS_URL || DEFAULTS.FAIRSCALE_TRUST_EVENTS_URL!,
    FAIRSCALE_TRUST_EVENTS_KEY:
      process.env.FAIRSCALE_TRUST_EVENTS_KEY || DEFAULTS.FAIRSCALE_TRUST_EVENTS_KEY!,
    ...
    FAIRSCALE_TRUST_EVENTS_BATCH_SIZE: parseInt(
      process.env.FAIRSCALE_TRUST_EVENTS_BATCH_SIZE ||
        String(DEFAULTS.FAIRSCALE_TRUST_EVENTS_BATCH_SIZE),
      10
    ),
    FAIRSCALE_TRUST_EVENTS_FLUSH_INTERVAL_MS: parseInt(
      process.env.FAIRSCALE_TRUST_EVENTS_FLUSH_INTERVAL_MS ||
        String(DEFAULTS.FAIRSCALE_TRUST_EVENTS_FLUSH_INTERVAL_MS),
      10
    ),
    FAIRSCALE_TRUST_EVENTS_TIMEOUT_MS: parseInt(
      process.env.FAIRSCALE_TRUST_EVENTS_TIMEOUT_MS ||
        String(DEFAULTS.FAIRSCALE_TRUST_EVENTS_TIMEOUT_MS),
      10
    ),
    ...
    FAIRSCALE_TRUST_EVENTS_MAX_RETRY_MS: parseInt(
      process.env.FAIRSCALE_TRUST_EVENTS_MAX_RETRY_MS ||
...
export function getRedactedConfig(): Record<string, string> {
  ...
  return {
    ...
    FAIRSCALE_TRUST_EVENTS_URL: config.FAIRSCALE_TRUST_EVENTS_URL,
    FAIRSCALE_TRUST_EVENTS_KEY: config.FAIRSCALE_TRUST_EVENTS_KEY ? '[REDACTED]' : '',
