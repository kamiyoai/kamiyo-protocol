...
import db from '../db';
import type { AgentVariant } from './service';
import { listActiveVariants, thompsonSample } from './service';
import { createTournament } from './tournament';
import { banditDecisionsTotal, banditSweepPromotionsTotal } from '../metrics';
...
export function routeVariant(
  ...
  if (active.length === 0 && !promoted) {
    banditDecisionsTotal.inc({ task_type: taskType, strategy: 'none', result: 'empty' });
    return null;
  }
  ...
  banditDecisionsTotal.inc({ task_type: taskType, strategy: 'thompson', result: 'ok' });
  return {
    variant: picked,
...
function fromPromoted(taskType: string, row: Record<string, unknown>): RouteDecision {
  const tournament = getOrCreateStandingTournament(taskType);
  banditDecisionsTotal.inc({ task_type: taskType, strategy: 'promoted', result: 'ok' });
  return {
    variant: {
      id: row.id as string,
...
export async function sweepPromotions(
  ...
  for (const taskType of listTaskTypes()) {
    ...
    if (r.promoted) {
      banditSweepPromotionsTotal.inc({ task_type: taskType, result: 'promoted' });
      results.push({ taskType, promoted: true, variantId: r.variantId });
    } else {
      banditSweepPromotionsTotal.inc({ task_type: taskType, result: 'skipped' });
