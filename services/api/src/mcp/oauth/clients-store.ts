/**
 * OAuth Registered Clients Store
...
function dbToClientInfo(row: McpOAuthClient, includeSecret = false): OAuthClientInformationFull {
  return {
    client_id: row.client_id,
    client_secret: includeSecret ? undefined : undefined, // Never expose stored secret
...
export class KamiyoOAuthClientsStore implements OAuthRegisteredClientsStore {
  /**
   * Get client by ID. Used for token validation.
   */
  getClient(clientId: string): OAuthClientInformationFull | undefined {
    const row = getMcpOAuthClient(clientId);
    if (!row) return undefined;
    return dbToClientInfo(row);
  }
  ...
  registerClient(
    ...
    return {
      client_id: clientId,
      client_secret: clientSecret, // Return secret only on registration
  ...
  /**
   * Verify client secret matches stored hash
   */
  verifyClientSecret(clientId: string, secret: string): boolean {
    const row = getMcpOAuthClient(clientId);
    if (!row) return false;
    return row.client_secret_hash === hashSecret(secret);
  }
}
