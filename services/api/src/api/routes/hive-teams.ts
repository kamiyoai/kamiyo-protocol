...
router.get('/', (req: Request, res: Response) => {
  ...
  const typedTeams = teams as Array<{
    ...
    daily_limit: number; pool_balance: number; pool_balance_sol: number;
  ...
  res.json({
    teams: typedTeams.map((t) => ({
      ...
      poolBalanceSol: t.pool_balance_sol,
...
// DELETE /api/hive-teams/:id - requires owner
...
router.delete('/:id', requireTeamOwner, async (req: Request, res: Response) => {
  ...
  const team = db.prepare('SELECT id, pool_balance, pool_balance_sol, currency FROM swarm_teams WHERE id = ?').get(teamId) as {
    id: string; pool_balance: number; pool_balance_sol: number; currency: string;
  } | undefined;
  if (!team) {
    res.status(404).json({ error: 'Team not found' });
    return;
  ...
  if (refundAmountSol > 0) {
    ...
    if (treasuryKeypair) {
      try {
        ...
        const lamports = Math.floor(refundAmountSol * LAMPORTS_PER_SOL);
        const tx = new Transaction().add(
          SystemProgram.transfer({
            fromPubkey: HIVE_TREASURY,
        ...
        refundSignatureSol = await connection.sendRawTransaction(tx.serialize(), {
          skipPreflight: false,
          preflightCommitment: 'confirmed',
        });
        await connection.confirmTransaction({ signature: refundSignatureSol, blockhash, lastValidBlockHeight }, 'confirmed');
      } catch (err) {
        console.error('SOL refund failed:', err);
        // Continue with deletion even if SOL refund fails
      }
    }
  }
...
// DELETE /api/hive-teams/:id/members/:memberId - requires owner
...
// POST /api/hive-teams/:id/fund-test — Test endpoint to credit pool without payment
...
// POST /api/hive-teams/:id/fund-tokens — Fund pool with actual $KAMIYO tokens
router.post('/:id/fund-tokens', requireTeamOwner, async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      ...
      poolBalanceSol: updated.pool_balance_sol,
      tokenAmount,
      signature,
    });
  } catch (err) {
...
// POST /api/hive-teams/:id/fund-sol — Fund pool with native SOL
router.post('/:id/fund-sol', requireTeamOwner, async (req: Request, res: Response) => {
  const teamId = req.params.id;
  const { signedTransaction } = req.body;
  const wallet = req.auth?.wallet;
  ...
  try {
    ...
    // Parse SOL transfer amount from SystemProgram.transfer instruction
    let lamportsTransferred = 0n;
    for (const ix of transaction.instructions) {
      if (ix.programId.equals(SystemProgram.programId) && ix.data.length >= 12) {
        // SystemProgram.transfer instruction: 4 bytes type + 8 bytes lamports (little-endian)
        const instructionType = ix.data.readUInt32LE(0);
        if (instructionType === 2) { // Transfer = 2
          lamportsTransferred = ix.data.readBigUInt64LE(4);
          break;
        }
      }
    }

    if (lamportsTransferred === 0n) {
      res.status(400).json({ error: 'No valid SOL transfer found in transaction' });
      return;
    }

    const solAmount = Number(lamportsTransferred) / LAMPORTS_PER_SOL;

    if (solAmount < 0.01) {
      res.status(400).json({ error: 'Minimum funding amount is 0.01 SOL' });
      return;
    }

    // Update SOL balance (separate from KAMIYO pool_balance)
    db.prepare(`
      UPDATE swarm_teams SET pool_balance_sol = pool_balance_sol + ?, updated_at = unixepoch()
      WHERE id = ?
    ...
    console.error('SOL funding failed:', err);
    res.status(500).json({ error: err instanceof Error ? err.message : 'SOL transfer failed' });
  }
});

// PATCH /api/hive-teams/:id/budget - requires owner
...
// POST /api/hive-teams/:id/propose-task
...
// POST /api/hive-teams/:id/vote-bid
...
// POST /api/hive-teams/:id/reveal-bid
...
// POST /api/hive-teams/:id/execute-proposal
...
// GET /api/hive-teams/:id/proposals
...
// GET /api/hive-teams/:id/proposals/:proposalId
...
function getTeamDetail(teamId: string) {
  ...
  return {
    ...
    poolBalanceSol: team.pool_balance_sol,
