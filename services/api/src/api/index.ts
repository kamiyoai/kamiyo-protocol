...
export function createApiServer(config: ApiServerConfig = {}): Express {
  ...
  app.get('/health', (_req, res) => {
    res.json({ status: 'ok', service: 'kamiyo-api' });
  });
  ...
  // Holder API auth routes (no auth required)
...
const openApiSpec = {
  ...
  info: {
    title: 'KAMIYO API',
    version: '1.0.0',
    description: 'Consolidated API for KAMIYO protocol - Holder API, Blindfold verification, and X bot',
  },
  servers: [
