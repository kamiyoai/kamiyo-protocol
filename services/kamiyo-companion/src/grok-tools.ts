...
import { grokClient } from './clients';
...
// Perform search using Grok tool calling
  ...
  try {
    ...
    if (message.tool_calls && message.tool_calls.length > 0) {
      const toolNames = message.tool_calls
        .filter((tc): tc is OpenAI.Chat.Completions.ChatCompletionMessageToolCall & { function: { name: string } } =>
...
// Search X for specific handles' recent activity
export async function searchXHandles(
  ...
  const handleList = handles.slice(0, 10).map(h => `@${h}`).join(', ');
...
// Check if Grok tools are available
