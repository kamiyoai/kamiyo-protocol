...
export function formatContextForPrompt(ctx: MarketContext): string {
  ...
  if (ctx.btcPrice) {
    lines.push(
      `BTC: $${ctx.btcPrice.toLocaleString()} | ETH: $${ctx.ethPrice?.toLocaleString() || '?'}`
    );
  }

  if (ctx.marketSentiment) {
    const mood =
      ctx.marketSentiment === 'greed'
        ? 'bullish'
        : ctx.marketSentiment === 'fear'
          ? 'bearish'
          : 'sideways';
...
export function startContextRefresh(): void {
  ...
  refreshContext().catch((err) =>
    logger.error('Initial context refresh failed', { error: String(err) })
  );
