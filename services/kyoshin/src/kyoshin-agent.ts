...
const log = createLogger('kyoshin:agent');
...
export class NikaAgent {
  ...
  constructor(config: NikaAgentConfig) {
    ...
    log.info('Kyoshin agent initialized', { dkgEnabled: !!this.dkgMemory });
  }
  ...
  async generateReply(
    ...
    const prompt = `A user mentioned Kyoshin on Twitter.
...
TASK:
Craft a thoughtful reply fitting Kyoshin's voice.
