/**
 * Task Completion Publisher - Publishes Meishi-compatible compliance audits for Kyoshin output.
 *
 * Converts Kyoshin's published tweets into `schema:Review` audit assets so they
...
const log = createLogger('kyoshin:task-publisher');
    ...
    const ual = await publishCircuit.execute(() =>
      withRetry(
        async () => dkgMemory.storeComplianceAudit({
          ...
          auditType: 'periodic',
          jurisdiction: 'global',
          summary: 'Kyoshin service heartbeat audit for Meishi visibility and DKG liveness.',
          source: 'kyoshin-task-publisher-heartbeat',
