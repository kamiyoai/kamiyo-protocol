...
const OFFERINGS: OfferingRegistry = {
  nika_tweet: {
    ...
    requestPayment: () => 'Accepted. Pay to receive one Kyoshin tweet (<=280 chars).',
    execute: async ({ prompt }) => ({ deliverable: await generateNikaTweet(prompt) }),
  },
  nika_thread: {
    validate: (request) => parseOrReject(ThreadRequest, request),
    requestPayment: () => 'Accepted. Pay to receive a short Kyoshin thread (3-7 tweets) + alternates.',
  ...
  nika_reply_pack: {
    ...
    requestPayment: () => 'Accepted. Pay to receive a Kyoshin reply pack (4-12 replies).',
  ...
  nika_research_brief: {
    ...
    requestPayment: () => 'Accepted. Pay to receive a Kyoshin research brief (thesis, counters, next steps).',
  ...
  nika_launch_pack: {
    ...
    requestPayment: () => 'Accepted. Pay to receive a full Kyoshin launch kit (positioning + tweets + thread + replies).',
