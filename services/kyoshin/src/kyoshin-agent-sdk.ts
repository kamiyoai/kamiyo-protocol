/**
 * Kyoshin Agent - Claude Agent SDK with debate orchestrator.
 */
...
const log = createLogger('kyoshin:agent-sdk');
 ...
 * - Synthesizer: Combines all perspectives into Kyoshin's voice
 */
const NIKA_SUBAGENTS: Record<string, AgentDefinition> = {
  oracle: {
    ...
    prompt: `You are the Oracle perspective for Kyoshin's multi-perspective content generation.
...
Be precise, cite specifics when possible. No speculation.`,
  ...
  philosopher: {
    ...
    prompt: `You are the Philosopher perspective for Kyoshin's multi-perspective content generation.
...
Be thoughtful, find the non-obvious angles. Question assumptions.`,
  ...
  provocateur: {
    ...
    prompt: `You are the Provocateur perspective for Kyoshin's multi-perspective content generation.
...
Be bold, but intellectually honest. Provoke thought, not anger.`,
  ...
  synthesizer: {
    description: "Final voice. Combines all perspectives into Kyoshin's authentic tweet.",
    prompt: `You are the Synthesizer for Kyoshin's multi-perspective content generation.

You receive analyses from Oracle (data), Philosopher (meaning), and Provocateur (challenge).
Your job is to create Kyoshin's FINAL TWEET that weaves these perspectives together.
...
- You are Kyoshin (二化), not "KAMIYO Companion"
...
export class NikaAgentSDK {
  ...
  constructor(config: NikaAgentSDKConfig) {
    ...
    log.info('Kyoshin Agent SDK initialized', {
  ...
  async generateReply(
    ...
    // @kamiyoai is the parent account that automates Kyoshin - just acknowledge
...
Just the acknowledgment, nothing else. Return ONLY the acknowledgment text.`
      : `Reply to this mention as Kyoshin.
...
Output ONLY the reply text.`;
  ...
  async generateSkillReply(input: {
    ...
    const prompt = `You are Kyoshin (二化). A holder asked you to run a skill.
...
- Return ONLY the final answer text (no preamble, no reasoning).`;
  ...
  async generateQuote(
    ...
    const prompt = `Generate a quote tweet for Kyoshin (二化).
...
Return ONLY the quote text.`;
  ...
  private buildPostPrompt(
    ...
    let prompt = `Generate a tweet for Kyoshin (二化) using multi-perspective analysis.
