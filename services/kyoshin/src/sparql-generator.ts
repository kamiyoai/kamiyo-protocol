...
const log = createLogger('kyoshin:sparql-gen');
...
/**
 * Kyoshin's ontology schema for SPARQL generation
 */
const NIKA_ONTOLOGY = `
...
# Kyoshin's Knowledge Graph Schema:

## Classes:
- schema:SocialMediaPosting - A tweet (original post)
- sioc:Reply - A reply to another tweet
- kyoshin:Observation - Something Kyoshin observed but didn't tweet

## Common Properties:
- schema:articleBody - The text content
- schema:author - The author (always Kyoshin for her posts)
- schema:datePublished - ISO timestamp
- schema:keywords - Array of topic keywords
- kyoshin:assetType - "tweet" | "reply" | "quote" | "observation"
- kyoshin:mood - Mood when creating (curious, analytical, playful, etc.)
- kyoshin:tweetId - Twitter ID if posted
- kyoshin:inReplyTo - Tweet ID being replied to
- kyoshin:quotedTweet - Tweet ID being quoted
- kyoshin:confidence - Confidence score for observations (0-1)
...
WHERE {
  ?ual kyoshin:assetType "observation" .
  ?ual schema:description ?content .
  ?ual kyoshin:confidence ?confidence .
  ...
  async generateQuery(naturalLanguage: string): Promise<SPARQLGenerationResult> {
    ...
    try {
      const response = await this.client.messages.create({
        ...
        max_tokens: 1024,
        system: `You are a SPARQL query generator for Kyoshin's knowledge graph.
  ...
  async answerQuestion(question: string): Promise<{
    ...
    try {
      ...
      const response = await this.client.messages.create({
        model: this.config.model || 'claude-haiku-4-5-20251001',
        max_tokens: 500,
        system: `You are Kyoshin, answering questions based ONLY on your stored knowledge.
