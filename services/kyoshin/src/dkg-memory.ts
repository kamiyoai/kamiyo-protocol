...
const log = createLogger('kyoshin:dkg-memory');
...
export class DKGMemory {
  ...
  async storeReply(params: {
    ...
    const asset = {
      '@context': {
        ...
        kyoshin: 'https://kamiyo.ai/ontology/kyoshin/',
      },
      '@type': 'sioc:Reply',
      'schema:articleBody': params.content,
      'schema:author': {
        '@type': 'schema:Person',
        'schema:name': 'Kyoshin (二化)',
      ...
      'kyoshin:assetType': 'reply',
      'kyoshin:tweetId': params.replyId,
      'kyoshin:inReplyTo': params.inReplyTo,
    };
  ...
  async storeQuote(params: {
    ...
    const asset = {
      ...
      'kyoshin:assetType': 'quote',
      'kyoshin:tweetId': params.quoteId,
      'kyoshin:quotedTweet': params.quotedTweetId,
    };
  ...
  /**
   * Store an observation (something Kyoshin noticed but didn't tweet about).
   */
  async storeObservation(params: {
    ...
    const asset = {
      ...
      '@type': 'kyoshin:Observation',
      'schema:description': params.content,
      'schema:dateCreated': new Date().toISOString(),
      'kyoshin:source': params.source,
      'kyoshin:confidence': params.confidence,
  ...
  async storeComplianceAudit(params: {
    ...
    const asset: Record<string, unknown> = {
      ...
      itemReviewed: {
        ...
        name: `Kyoshin (${this.config.twitterHandle})`,
      },
      author: {
        '@type': 'Organization',
        identifier: `kyoshin:${this.config.twitterHandle}`,
        name: 'Kyoshin',
      },
      reviewRating: {
        '@type': 'Rating',
      ...
      additionalProperty: [
        ...
        propertyValue('source', params.source || 'kyoshin-task-publisher'),
    ...
    return this.createAsset(asset, {
      ...
      timestamp: new Date().toISOString(),
      tweetId: params.tweetId,
      topics: ['ComplianceAudit', 'meishi', 'kyoshin'],
    });
  }
  ...
  async searchRecent(params: {
    ...
    const sparql = `
      PREFIX schema: <https://schema.org/>
      PREFIX kyoshin: <https://kamiyo.ai/ontology/kyoshin/>
      PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>

      SELECT ?ual ?content ?type ?date
      WHERE {
        ?ual schema:articleBody ?content .
        ?ual kyoshin:assetType ?type .
  ...
  /**
   * Get Kyoshin's recent tweet topics (for avoiding repetition).
   */
  ...
  async getEngagementPatterns(days = 7): Promise<{
    ...
    const sparql = `
      ...
      WHERE {
        ?ual kyoshin:assetType "tweet" .
        ?ual schema:datePublished ?date .
        OPTIONAL { ?ual kyoshin:mood ?mood }
        FILTER (xsd:dateTime(?date) >= xsd:dateTime("${since.toISOString()}"))
      }
      GROUP BY ?mood
      ORDER BY DESC(?count)
  ...
  async updateEngagement(
    ...
    const sparql = `
      ...
      SELECT ?ual
      WHERE {
        ?ual kyoshin:tweetId "${sanitizeForSPARQL(tweetId)}" .
      }
      LIMIT 1
    `;
  ...
  private buildTweetAsset(params: {
    ...
    return {
      ...
      'schema:datePublished': new Date().toISOString(),
      'schema:keywords': params.topics || this.extractKeywords(params.content),
      'kyoshin:assetType': 'tweet',
      'kyoshin:mood': params.mood,
      'kyoshin:tweetId': params.tweetId,
    };
  }

  private extractMetadata(content: Record<string, unknown>): MemoryMetadata {
    return {
      type: (content['kyoshin:assetType'] as MemoryType) || 'observation',
      timestamp: (content['schema:datePublished'] as string) || new Date().toISOString(),
      tweetId: content['kyoshin:tweetId'] as string | undefined,
      mood: content['kyoshin:mood'] as string | undefined,
      inReplyTo: content['kyoshin:inReplyTo'] as string | undefined,
      quotedTweet: content['kyoshin:quotedTweet'] as string | undefined,
    };
  }
