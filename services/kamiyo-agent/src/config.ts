...
const envSchema = z.object({
  ...
  KAMIYO_DB_PATH: z.string().default('output/kamiyo-agent/state.db'),
  KAMIYO_OUTBOX_DIR: z.string().default('output/kamiyo-agent/outbox'),
  ...
  KAMIYO_SWARM_NEAR_MARKET_BID_DISCOUNT_BPS: z.coerce
    .number()
    .int()
    .min(1)
    .max(10_000)
  ...
  KAMIYO_SWARM_NEAR_MARKET_PROPOSAL_TEMPLATE: z
    .string()
    .default(
      'Autonomous delivery with proof artifacts, deterministic output, and deadline compliance.'
    ),
  ...
  KAMIYO_SWARM_NEAR_MARKET_SETTLEMENT_ENABLED: z
    ...
    .default('true')
    .transform(v => v === 'true'),
  KAMIYO_SWARM_NEAR_MARKET_SETTLEMENT_INTERVAL_MINUTES: z.coerce
    .number()
    .int()
    .positive()
    .default(10),
  ...
  KAMIYO_SWARM_NEAR_MARKET_SUBMIT_RETRY_BACKOFF_MINUTES: z.coerce
    .number()
    .int()
    .positive()
    .default(10),
  KAMIYO_SWARM_NEAR_MARKET_SUBMIT_RETRY_MAX_BACKOFF_MINUTES: z.coerce
    .number()
    .int()
    .positive()
    .default(180),
  KAMIYO_SWARM_NEAR_MARKET_SUBMIT_ESCALATE_AFTER_MINUTES: z.coerce
    .number()
    .int()
    .positive()
    .default(45),
  ...
  KAMIYO_SWARM_NEAR_MARKET_WITHDRAW_PENDING_MAX_MINUTES: z.coerce
    .number()
    .int()
    .positive()
    .default(30),
  ...
  KAMIYO_INSIGHT_EXTRACTION_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_TICK_CHECKPOINT_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_TICK_CHECKPOINT_MAX_STALE_MS: z.coerce.number().int().positive().default(600_000),
  KAMIYO_STREAMING_EVENTS_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_EVENTS_SSE_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_AGENT_MEMORY_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_AGENT_MEMORY_MAX_PER_TYPE: z.coerce.number().int().positive().default(20),
  KAMIYO_AGENT_MEMORY_INJECTION_LIMIT: z.coerce.number().int().positive().default(5),
  KAMIYO_MANDATE_CLASSIFICATION_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_AGENTIC_LOOP_ENABLED: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  ...
  KAMIYO_AGENTIC_LOOP_BASE_URL: optionalNonEmptyString,
  KAMIYO_AGENTIC_LOOP_MODEL: z.string().default('gpt-4o-mini'),
  ...
  KAMIYO_AGENT_TEAMS_ENABLED: z
    ...
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_AGENT_TEAMS_REQUIRE_SCOUT: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),
  KAMIYO_AGENT_TEAMS_REQUIRE_VERIFIER: z
    .enum(['true', 'false'])
    .default('false')
    .transform(v => v === 'true'),

  KAMIYO_AGENT_HTTP_ENABLED: z
    .enum(['true', 'false'])
    .default('true')
    .transform(v => v === 'true'),
  KAMIYO_AGENT_HTTP_HOST: z.string().min(1).default('0.0.0.0'),
  KAMIYO_AGENT_HTTP_PORT: z.coerce.number().int().positive().default(4020),
  KAMIYO_AGENT_HTTP_TOKEN: optionalNonEmptyString,
});
