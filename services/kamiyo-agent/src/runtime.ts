...
function readExecutionPolicyEnvOverrides(envFilePath: string): Partial<ExecutionPolicyInput> {
  ...
  const kamiyoAgentAutoClaimEnabled = parseBooleanLiteral(kv.KAMIYO_AGENT_AUTO_CLAIM_ENABLED);
  if (kamiyoAgentAutoClaimEnabled != null) {
    overrides.KAMIYO_AGENT_AUTO_CLAIM_ENABLED = kamiyoAgentAutoClaimEnabled;
  }
...
function executionPolicyFingerprint(policy: ExecutionPolicy): string {
  ...
  return JSON.stringify({
    ...
    kamiyoAgentAutoClaimEnabled: policy.kamiyoAgentAutoClaimEnabled,
...
export class KamiyoAgentRuntime {
  ...
  getMetrics(): string {
    ...
    const lines = [
      '# HELP kamiyo_agent_process_up Process health state.',
      '# TYPE kamiyo_agent_process_up gauge',
      'kamiyo_agent_process_up 1',
      '# HELP kamiyo_agent_tick_total Tick totals by status.',
      '# TYPE kamiyo_agent_tick_total gauge',
      `kamiyo_agent_tick_total{status="ok"} ${tickStats.ok}`,
      `kamiyo_agent_tick_total{status="error"} ${tickStats.error}`,
      `kamiyo_agent_tick_total{status="running"} ${tickStats.running}`,
      '# HELP kamiyo_agent_swarm_non_intervention_rate Fraction of ticks without intervention.',
      '# TYPE kamiyo_agent_swarm_non_intervention_rate gauge',
      `kamiyo_agent_swarm_non_intervention_rate ${sloReport.metrics.nonInterventionRate}`,
      '# HELP kamiyo_agent_swarm_route_success_rate Route action success ratio.',
      '# TYPE kamiyo_agent_swarm_route_success_rate gauge',
      `kamiyo_agent_swarm_route_success_rate ${sloReport.metrics.routeSuccessRate}`,
      '# HELP kamiyo_agent_swarm_decision_loop_uptime Decision loop uptime ratio.',
      '# TYPE kamiyo_agent_swarm_decision_loop_uptime gauge',
      `kamiyo_agent_swarm_decision_loop_uptime ${sloReport.metrics.decisionLoopUptime}`,
      '# HELP kamiyo_agent_treasury_spent_today_sol Estimated spend since UTC day start.',
      '# TYPE kamiyo_agent_treasury_spent_today_sol gauge',
      `kamiyo_agent_treasury_spent_today_sol ${this.status.treasury.spentTodaySol}`,
      '# HELP kamiyo_agent_treasury_tx_today Number of treasury tx attempts today.',
      '# TYPE kamiyo_agent_treasury_tx_today gauge',
      `kamiyo_agent_treasury_tx_today ${this.status.treasury.txToday}`,
      '# HELP kamiyo_agent_intake_jobs_pending Pending intake jobs.',
      '# TYPE kamiyo_agent_intake_jobs_pending gauge',
      `kamiyo_agent_intake_jobs_pending ${this.status.economics.pendingIntakeJobs}`,
      '# HELP kamiyo_agent_revenue_net_today_sol Net realized revenue for UTC day.',
      '# TYPE kamiyo_agent_revenue_net_today_sol gauge',
      `kamiyo_agent_revenue_net_today_sol ${this.status.economics.netRevenueTodaySol}`,
      '# HELP kamiyo_agent_self_improve_effective_min_margin_sol Adaptive min margin.',
      '# TYPE kamiyo_agent_self_improve_effective_min_margin_sol gauge',
      `kamiyo_agent_self_improve_effective_min_margin_sol ${this.status.selfImprove.effectiveMinMarginSol}`,
      '# HELP kamiyo_agent_self_improve_effective_executions_per_tick Adaptive execution rate.',
      '# TYPE kamiyo_agent_self_improve_effective_executions_per_tick gauge',
      `kamiyo_agent_self_improve_effective_executions_per_tick ${this.status.selfImprove.effectiveExecutionsPerTick}`,
    ];
  ...
  private getExecutionPolicyInput(): ExecutionPolicyInput {
    return {
      ...
      KAMIYO_AGENT_AUTO_CLAIM_ENABLED: this.executionPolicy.kamiyoAgentAutoClaimEnabled,
  ...
  private async runTick(tickId: string): Promise<void> {
    ...
    if (this.runtimeEnv.KAMIYO_MODE === 'execute') {
      ...
      budget = await this.maybeClaimKamiyoAgentStaking({ tickId, dayStartIso, budget });
      ...
      if (this.executionPolicy.autoStakeEnabled && this.runtimeEnv.KAMIYO_AGENT_STAKING_POOL) {
        const claimSigner = loadOptionalKeypair({
          keypairPath: this.runtimeEnv.KAMIYO_AGENT_CLAIMER_KEYPAIR_PATH,
          privateKey: this.runtimeEnv.KAMIYO_AGENT_CLAIMER_PRIVATE_KEY,
        ...
        budget = await this.maybeRouteToStakingPool({
          ...
          dayStartIso,
          budget,
          poolAddress: this.runtimeEnv.KAMIYO_AGENT_STAKING_POOL,
          depositor: claimSigner.keypair,
          source: 'runtime_auto_stake_kamiyo-agent',
        });
      }
  ...
  private async buildNearMarketDeliverable(params: {
    ...
    const meta = [
      ...
      '- Generated by: kamiyo-agent-exec autonomous runner',
    ].join('\n');
  ...
  private async maybeClaimKamiyoAgentStaking(params: {
    tickId: string;
    dayStartIso: string;
    budget: BudgetState;
  }): Promise<BudgetState> {
    if (
      !this.runtimeEnv.KAMIYO_AGENT_STAKING_POOL ||
      !this.executionPolicy.kamiyoAgentAutoClaimEnabled
    ) {
      return params.budget;
    }
    ...
    const periodNumbers = getClaimablePeriodNumbers(
      position,
      this.runtimeEnv.KAMIYO_AGENT_AUTO_CLAIM_MAX_PERIODS_PER_RUN
    );

    if (periodNumbers.length === 0) return params.budget;
    if (claimableLamports < BigInt(this.runtimeEnv.KAMIYO_AGENT_AUTO_CLAIM_MIN_LAMPORTS))
      return params.budget;
    ...
    this.db.addAction(
      ...
      {
        source: 'runtime_kamiyo_agent_staking_claim',
        pool: this.runtimeEnv.KAMIYO_AGENT_STAKING_POOL,
    ...
    const receiptPath = writeOutbox(
      resolvePath(this.runtimeEnv.KAMIYO_OUTBOX_DIR),
      'kamiyo-agent-staking-claim-receipt',
      {
        tickId: params.tickId,
        at: new Date().toISOString(),
        source: 'runtime_kamiyo_agent_staking_claim',
    ...
    this.db.recordRevenueEvent({
      id: `${params.tickId}:claim:kamiyo-agent:${this.runtimeEnv.KAMIYO_AGENT_STAKING_POOL}`,
      tickId: params.tickId,
      lane: 'internal',
      kind: 'claim',
      amountSol: claimSol,
