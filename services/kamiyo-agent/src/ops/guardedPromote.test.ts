...
const GUARDED_PROMOTE_PATH = resolve(
  fileURLToPath(new URL('../../../../ops/kamiyo-agent-exec/guarded-promote.sh', import.meta.url))
);
...
function setupHarness(
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'kamiyo-agent-guarded-promote-'));
  t.after(() => {
    rmSync(tempDir, { recursive: true, force: true });
  });

  const envFile = join(tempDir, 'kamiyo-agent-exec.env');
  ...
  writeFileSync(
    ...
    [
      ...
      'KAMIYO_AGENT_HTTP_HOST=127.0.0.1',
      'KAMIYO_AGENT_HTTP_PORT=4020',
      '',
    ].join('\n')
  );
