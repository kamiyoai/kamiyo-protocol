...
async function main() {
  console.log(`
  ${c.m('╔═══════════════════════════════════════════════════════════════════════════╗')}
  ${c.m('║')}  ${c.h('KAMIYO')} ${c.m('×')} ${c.p('DARK_FOREST')}    ${c.w('Zero-Knowledge Reputation Proof')}                       ${c.m('║')}
...
`);
  ...
  if (!fs.existsSync(wasmPath)) {
    console.log('  [!] Circuit artifacts not found. Run: cd packages/kamiyo-dark-forest-privacy/circuits && ./build.sh\n');
    process.exit(1);
  }
