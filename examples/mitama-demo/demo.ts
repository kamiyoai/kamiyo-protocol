...
async function main() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const identityCommitment = MitamaProver.generateIdentityCommitment(
      ownerSecret,
      agentId
    );
  ...
  for (let i = 0; i < agents.length; i++) {
    ...
    const signalCommitment = MitamaProver.generateSignalCommitment(
      signal.type,
    ...
    const nullifier = MitamaProver.generateNullifier(
      agent.ownerSecret,
      BigInt(1) // epoch 1
    );
  ...
  const actionHash = MitamaProver.generateActionHash(
    1, // EXECUTE_TRADE
  ...
  for (let i = 0; i < agents.length; i++) {
    ...
    const voteNullifier = MitamaProver.generateNullifier(
      agent.ownerSecret,
