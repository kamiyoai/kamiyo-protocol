...
import { MitamaClient, MerkleTree, generateAgentId } from '@kamiyo/hive';
...
async function main() {
  ...
  // Convert merkle proof to bigints
  const merkleProof = {
    path: merkleProofBytes.map(p => bytesToBigint(p)),
    indices: pathIndices,
  };
  ...
  try {
    const { proof, voteNullifier, voteCommitment } = await proveSwarmVote({
      agentsRoot: bytesToBigint(registry.agentsRoot),
      ownerSecret,
      agentId,
      registrationSecret,
      merkleProof,
