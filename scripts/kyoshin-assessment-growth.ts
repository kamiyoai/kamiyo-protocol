...
const DEFAULT_STATE_PATH = 'output/kyoshin/assessment-growth-state.json';
...
function createTransactionId(index: number): string {
  return `kyoshin-assess-${Date.now().toString(36)}-${index.toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}
...
function log(level: 'info' | 'warn' | 'error', message: string, context?: Record<string, unknown>): void {
  const payload = {
    ...
    level,
    service: 'kyoshin-assessment-growth',
    message,
    ...(context ? { context } : {}),
  };
