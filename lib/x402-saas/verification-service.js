...
export class VerificationService {
  ...
  static async verifyPayment(apiKey, txHash, chain, expectedAmount = null, ipAddress = null) {
    ...
    // 5. Record usage
    await TenantManager.recordVerification(tenantId);

    // 6. Store verification for analytics
    await this.recordVerification(tenantId, {
      txHash,
      chain,
   ...
   * @returns {Promise<object>}
   */
  static async recordVerification(tenantId, data) {
    return prisma.x402Verification.create({
      data: {
        tenantId,
        ...data,
