...
import { getSimulatedStats } from './simulated-stats';
...
// Get fallback stats from simulated system (call function for fresh values)
...
export async function fetchProtocolStats() {
    ...
    try {
        ...
        return {
            totalAssessments: totalDisputes || fallbackStats.totalAssessments,
            completed: resolved || fallbackStats.completed,
            avgQuality,
            totalRefunded: totalRefunded > 0 ? totalRefunded.toFixed(2) : fallbackStats.totalRefunded,
            distribution,
            isSimulated: false
        };
    } catch (error) {
