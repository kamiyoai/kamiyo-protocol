...
template SwarmVote(TREE_DEPTH) {
    ...
    // 4. Verify nullifier is correctly computed (prevents double-voting)
    component nullifierHasher = Poseidon(3);
    nullifierHasher.inputs[0] <== agent_id;
    nullifierHasher.inputs[1] <== registration_secret;
    nullifierHasher.inputs[2] <== action_hash;
