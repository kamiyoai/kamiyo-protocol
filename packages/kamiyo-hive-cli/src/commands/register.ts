...
import { HiveProgram } from '../client/program.js';
import {
  showSuccess,
  showError,
  showInfo,
...
import { createNewIdentity, loadIdentity, HiveIdentity } from '../client/identity.js';

export interface AgentIdentity {
  commitment: Uint8Array;
  pda: string;
  secrets?: HiveIdentity;
}
...
export async function handleRegister(
  ...
  try {
    ...
    const [agentPDA] = HiveProgram.getAgentPDA(commitment);
    ...
    showSuccess('Identity secrets stored in ~/.hive/identity.json');
