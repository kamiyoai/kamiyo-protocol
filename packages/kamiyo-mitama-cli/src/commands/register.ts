...
import { createNewIdentity, loadIdentity, MitamaIdentity } from '../client/identity.js';

export interface AgentIdentity {
  commitment: Uint8Array;
  pda: string;
  secrets?: MitamaIdentity;
}
...
export async function handleRegister(
  ...
  try {
    ...
    const [agentPDA] = MitamaProgram.getAgentPDA(commitment);
    ...
    showSuccess('Identity secrets stored in ~/.mitama/identity.json');
