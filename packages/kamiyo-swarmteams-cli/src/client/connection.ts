...
export class SolanaClient {
  ...
  async loadWallet(password?: string): Promise<boolean> {
    ...
    // Try SwarmTeams encrypted wallet
    const swarmteamsPath = join(homedir(), '.swarmteams', 'wallet.enc.json');
    ...
    const legacyPath = join(homedir(), '.swarmteams', 'wallet.json');
  ...
  async createWallet(password: string): Promise<Keypair> {
    ...
    const swarmteamsDir = join(homedir(), '.swarmteams');
  ...
  async migrateUnencryptedWallet(password: string): Promise<boolean> {
    ...
    try {
      ...
      const newPath = join(homedir(), '.swarmteams', 'wallet.enc.json');
