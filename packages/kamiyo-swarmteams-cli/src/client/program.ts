...
export class SwarmTeamsProgram {
  ...
  private async getProgram(): Promise<Program> {
    ...
    // Load IDL from the swarmteams package
    const idlPath = new URL(
      '../../../swarmteams/src/idl/swarmteams.json',
      import.meta.url
    );
  ...
  async getRegistry(): Promise<RegistryState | null> {
    ...
    const [registryPDA] = SwarmTeamsProgram.getRegistryPDA();
  ...
  async getAgent(identityCommitment: Uint8Array): Promise<AgentState | null> {
    const program = await this.getProgram();
    const [agentPDA] = SwarmTeamsProgram.getAgentPDA(identityCommitment);
  ...
  async getSwarmAction(actionHash: Uint8Array): Promise<SwarmActionState | null> {
    const program = await this.getProgram();
    const [actionPDA] = SwarmTeamsProgram.getSwarmActionPDA(actionHash);
  ...
  async registerAgent(stakeAmount: bigint): Promise<{ signature: string; commitment: Uint8Array }> {
    ...
    const [registryPDA] = SwarmTeamsProgram.getRegistryPDA();
    const [stakeVault] = SwarmTeamsProgram.getStakeVaultPDA(registryPDA);
