...
const SWARMTEAMS_DIR = path.join(os.homedir(), '.swarmteams');
const IDENTITY_FILE = path.join(SWARMTEAMS_DIR, 'identity.json');
const SIGNALS_FILE = path.join(SWARMTEAMS_DIR, 'signals.json');

export interface SwarmTeamsIdentity {
...
function ensureSwarmTeamsDir(): void {
  if (!fs.existsSync(SWARMTEAMS_DIR)) {
    fs.mkdirSync(SWARMTEAMS_DIR, { recursive: true, mode: 0o700 });
  }
}

export function loadIdentity(network: string): SwarmTeamsIdentity | null {
  try {
...
export function saveIdentity(identity: SwarmTeamsIdentity): void {
  ensureSwarmTeamsDir();
...
export function createNewIdentity(
  ...
  const identity: SwarmTeamsIdentity = {
...
export function getIdentitySecrets(identity: SwarmTeamsIdentity): {
  ownerSecret: bigint;
