...
import { SwarmTeamsProgram } from '../client/program.js';
import {
  showSuccess,
  showError,
  showInfo,
...
import { createNewIdentity, loadIdentity, SwarmTeamsIdentity } from '../client/identity.js';

export interface AgentIdentity {
  commitment: Uint8Array;
  pda: string;
  secrets?: SwarmTeamsIdentity;
}

export async function handleRegister(
  client: SolanaClient,
  program: SwarmTeamsProgram,
  ...
  try {
    ...
    const [agentPDA] = SwarmTeamsProgram.getAgentPDA(commitment);
    ...
    showSuccess('Identity secrets stored in ~/.swarmteams/identity.json');
