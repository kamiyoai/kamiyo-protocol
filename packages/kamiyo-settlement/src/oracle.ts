...
export const COMMIT_PHASE_DURATION = 5 * 60 * 1000;
export const REVEAL_PHASE_DURATION = 30 * 60 * 1000;
...
export function computeCommitmentHash(
  ...
  if (!Number.isInteger(score) || score < 0 || score > 100) throw new Error('Score must be 0-100');
...
export function verifyCommitment(
  ...
  for (let i = 0; i < expected.length; i++) if (expected[i] !== commitment.commitmentHash[i]) return false;
  return true;
}

export function calculateConsensus(scores: number[]): ConsensusResult {
  if (scores.length < MIN_ORACLES) throw new Error(`At least ${MIN_ORACLES} oracle submissions required`);
  ...
  for (const score of sorted) {
    if (Math.abs(score - median) <= MAX_SCORE_DEVIATION) validScores.push(score);
    else outliers.push(score);
  }

  if (validScores.length < MIN_ORACLES) throw new Error('No consensus: too many outliers');

  const consensusScore = Math.round(validScores.reduce((a, b) => a + b, 0) / validScores.length);

  return { score: consensusScore, validScores, outliers, oracleCount: scores.length };
}
