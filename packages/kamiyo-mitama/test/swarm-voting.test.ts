...
import { MitamaProver, generateRandomSalt } from '../src';
...
describe('Swarm Voting Primitives', () => {
  describe('Vote Nullifier', () => {
    it('should generate unique nullifier per action', async () => {
      ...
      const n1 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, action1);
      const n2 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, action2);
    ...
    it('should generate same nullifier for same inputs', async () => {
      ...
      const n1 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, actionHash);
      const n2 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, actionHash);
    ...
    it('should prevent double voting on same action', async () => {
      ...
      const vote1 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, actionHash);
      const vote2 = await MitamaProver.generateVoteNullifier(agentId, registrationSecret, actionHash);
    ...
    it('should allow voting on multiple actions', async () => {
      ...
      const nullifiers = await Promise.all(
        actions.map((action) => MitamaProver.generateVoteNullifier(agentId, registrationSecret, action))
      );
  ...
  describe('Vote Commitment', () => {
    it('should hide vote value in commitment', async () => {
      ...
      const approveCommitment = await MitamaProver.generateVoteCommitment(true, voteSalt, actionHash);
      const rejectCommitment = await MitamaProver.generateVoteCommitment(false, voteSalt, actionHash);
    ...
    it('should be deterministic', async () => {
      ...
      const c1 = await MitamaProver.generateVoteCommitment(true, voteSalt, actionHash);
      const c2 = await MitamaProver.generateVoteCommitment(true, voteSalt, actionHash);
    ...
    it('should produce unique commitments with different salts', async () => {
      ...
      const c1 = await MitamaProver.generateVoteCommitment(true, salt1, actionHash);
      const c2 = await MitamaProver.generateVoteCommitment(true, salt2, actionHash);
  ...
  describe('Action Hash', () => {
    it('should hash action data deterministically', async () => {
      ...
      const h1 = await MitamaProver.generateActionHash(actionType, actionData);
      const h2 = await MitamaProver.generateActionHash(actionType, actionData);
    ...
    it('should produce different hashes for different action types', async () => {
      ...
      const h0 = await MitamaProver.generateActionHash(0, actionData);
      const h1 = await MitamaProver.generateActionHash(1, actionData);
      const h2 = await MitamaProver.generateActionHash(2, actionData);
    ...
    it('should produce different hashes for different action data', async () => {
      ...
      const h1 = await MitamaProver.generateActionHash(0, data1);
      const h2 = await MitamaProver.generateActionHash(0, data2);
    ...
    it('should handle variable length action data', async () => {
      ...
      const h1 = await MitamaProver.generateActionHash(0, shortData);
      const h2 = await MitamaProver.generateActionHash(0, longData);
  ...
  describe('Vote Reveal', () => {
    it('should generate consistent commitments for reveal verification', async () => {
      ...
      const commitment = await MitamaProver.generateVoteCommitment(
        voteValue,
        voteSalt,
        actionHash
      );
      ...
      const revealCommitment = await MitamaProver.generateVoteCommitment(
        voteValue,
        voteSalt,
        actionHash
      );
    ...
    it('should reject tampered vote reveals', async () => {
      ...
      const tamperedCommitment = await MitamaProver.generateVoteCommitment(
        false,
        voteSalt,
        actionHash
      );
    ...
    it('should reject wrong salt on reveal', async () => {
      ...
      const wrongSaltCommitment = await MitamaProver.generateVoteCommitment(
        true,
        wrongSalt,
        actionHash
      );
  ...
  describe('Vote Flow Simulation', () => {
    it('should support anonymous voting workflow', async () => {
      ...
      const actionHash = await MitamaProver.generateActionHash(0, actionData);
      ...
      const voteNullifier = await MitamaProver.generateVoteNullifier(
        agentId,
        registrationSecret,
        actionHash
      );
      ...
      const voteCommitment = await MitamaProver.generateVoteCommitment(
        voteValue,
        voteSalt,
        actionHash
      );
    ...
    it('should detect vote tampering via commitment verification', async () => {
      ...
      // Original vote: approve
      const originalCommitment = await MitamaProver.generateVoteCommitment(true, voteSalt, actionHash);

      // Trying to reveal as "reject" would fail
      const tamperedCommitment = await MitamaProver.generateVoteCommitment(false, voteSalt, actionHash);
