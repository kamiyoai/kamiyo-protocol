...
export async function scoreOutput(params: ScoreOutputInput): Promise<JudgeResult> {
  ...
  if (!judgeLLM) {
    recordRun({
      ...
      costUsd: 0,
      latencyMs: 0,
      error: 'judge LLM not configured',
    });
    return { ok: false, error: 'judge LLM not configured' };
  }
  ...
  try {
    ...
    if (!parsed) {
      recordRun({
        ...
        costUsd: cost,
        latencyMs,
        error: `unparseable: ${response.text.slice(0, 200)}`,
      });
...
function recordRun(run: {
  ...
  metrics.judgeCalls.inc({ task_type: run.taskType, result: run.status });
  if (run.latencyMs > 0) {
    metrics.judgeLatency.observe(
      { task_type: run.taskType, cache: run.cacheHit ? 'hit' : 'miss' },
      run.latencyMs / 1000
    );
  }
  if (run.costUsd > 0) {
    metrics.judgeCostUsd.inc({ task_type: run.taskType }, run.costUsd);
  }
}
