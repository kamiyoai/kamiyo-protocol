...
function getCircuitsDir(): string {
  ...
  const srcPath = path.resolve(__dirname, '../../../circuits/build/swarmteams');
  if (fs.existsSync(srcPath)) {
    return srcPath;
  }

  // 3. Relative to dist (dist/ after build)
  const distPath = path.resolve(__dirname, '../../../../circuits/build/swarmteams');
  if (fs.existsSync(distPath)) {
    return distPath;
  }

  // 4. Relative to workspace root
  const workspacePath = path.resolve(process.cwd(), 'circuits/build/swarmteams');
  if (fs.existsSync(workspacePath)) {
    return workspacePath;
  }

  // 5. Adjacent to node_modules (installed package)
  const installedPath = path.resolve(__dirname, '../circuits/build/swarmteams');
  if (fs.existsSync(installedPath)) {
    return installedPath;
  }

  throw new Error(
    'Circuit files not found. Set SWARMTEAMS_CIRCUITS_PATH or ensure circuits/build/swarmteams exists.'
  );
}
