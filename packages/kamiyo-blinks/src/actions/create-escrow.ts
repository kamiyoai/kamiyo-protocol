...
export function getCreateEscrowAction(requestUrl: URL): ActionGetResponse {
  ...
  if (provider) {
    for (const amount of ESCROW_CONFIG.QUICK_AMOUNTS) {
      actions.push({
        ...
        href: `${BASE_URL}/api/actions/create-escrow?provider=${encodeURIComponent(
          provider
        )}&amount=${encodeURIComponent(String(amount))}&timelock=24h`,
      });
    }
    ...
    actions.push({
      ...
      href: `${BASE_URL}/api/actions/create-escrow?provider=${encodeURIComponent(
        provider
      )}&amount={amount}&timelock={timelock}`,
      parameters: [
        {
          name: 'amount',
          label: 'Amount (SOL)',
...
export async function postCreateEscrow(
  ...
  const timelockSeconds =
    TIMELOCK_OPTIONS[timelockParam as keyof typeof TIMELOCK_OPTIONS] ??
  ...
  try {
    ...
    return {
      type: 'transaction',
      transaction,
      message: `Escrow created: ${amount} SOL for ${shortenAddress(
        providerParam
      )} (${formatTimeLock(timelockSeconds)} timelock). ID: ${escrowId}`,
      links: {
        next: {
          type: 'inline',
          action: {
            ...
            description:
              'Your escrow is active. Use the ID below to release or dispute.',
            label: 'Next Steps',
            links: {
              actions: [
                {
                  ...
                  href: `${BASE_URL}/api/actions/release-escrow?escrowId=${encodeURIComponent(
                    escrowId
                  )}&provider=${encodeURIComponent(providerParam)}`,
                },
                {
                  type: 'transaction',
                  label: 'File Dispute',
    ...
    throw new Error(`Create escrow failed: ${msg}`);
  }
}
