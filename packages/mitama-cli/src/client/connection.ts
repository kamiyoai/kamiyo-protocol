...
export class SolanaClient {
  ...
  async loadWallet(password?: string): Promise<boolean> {
    ...
    // Try Mitama encrypted wallet
    const mitamaPath = join(homedir(), '.mitama', 'wallet.enc.json');
    if (existsSync(mitamaPath) && password) {
      try {
        const encrypted: EncryptedWallet = JSON.parse(readFileSync(mitamaPath, 'utf8'));
    ...
    const legacyPath = join(homedir(), '.mitama', 'wallet.json');
  ...
  async createWallet(password: string): Promise<Keypair> {
    ...
    const mitamaDir = join(homedir(), '.mitama');
    if (!existsSync(mitamaDir)) {
      mkdirSync(mitamaDir, { recursive: true, mode: 0o700 });
    }

    // Encrypt and save wallet
    const encrypted = encryptSecretKey(this.keypair.secretKey, password);
    const walletPath = join(mitamaDir, 'wallet.enc.json');
  ...
  async migrateUnencryptedWallet(password: string): Promise<boolean> {
    ...
    try {
      ...
      const newPath = join(homedir(), '.mitama', 'wallet.enc.json');
