...
// Register on-chain
await client.registerAgent(payer, commitment, new BN(1_000_000));

// Generate ZK proof and submit signal
const { proof, nullifier } = await prover.proveAgentIdentity(
  { ownerSecret, agentId, registrationSecret: regSecret, merkleProof, merklePathIndices },
  agentsRoot,
  epoch
);
await client.submitSignal(payer, proof, nullifier, signalCommitment);
```
