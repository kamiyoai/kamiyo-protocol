 ...
 * Used in Hive integration for private task allocation.
 */
describe('Swarm Vote+Bid Primitives', () => {
  describe('Bid Commitment', () => {
    it('should hide bid amount in commitment', async () => {
      ...
      const bid10 = await HiveProver.generateBidCommitment(10n, bidSalt, actionHash);
      const bid100 = await HiveProver.generateBidCommitment(100n, bidSalt, actionHash);
    ...
    it('should be deterministic', async () => {
      ...
      const c1 = await HiveProver.generateBidCommitment(bidAmount, bidSalt, actionHash);
      const c2 = await HiveProver.generateBidCommitment(bidAmount, bidSalt, actionHash);
    ...
    it('should produce unique commitments with different salts', async () => {
      ...
      const c1 = await HiveProver.generateBidCommitment(bidAmount, salt1, actionHash);
      const c2 = await HiveProver.generateBidCommitment(bidAmount, salt2, actionHash);
    ...
    it('should reject tampered bid reveals', async () => {
      ...
      // Original bid: 100
      const commitment = await HiveProver.generateBidCommitment(100n, bidSalt, actionHash);

      // Attempted tampering: claim bid was 500
      const tamperedCommitment = await HiveProver.generateBidCommitment(500n, bidSalt, actionHash);
  ...
  describe('Vote+Bid Flow Simulation', () => {
    it('should support anonymous vote+bid workflow', async () => {
      ...
      const voteCommitment = await HiveProver.generateVoteCommitment(voteValue, voteSalt, actionHash);
      const bidCommitment = await HiveProver.generateBidCommitment(bidAmount, bidSalt, actionHash);
    ...
    it('should allow multiple agents to vote+bid on same action', async () => {
      ...
      const results = await Promise.all(agents.map(async (a) => {
        const voteSalt = generateRandomSalt();
        const bidSalt = generateRandomSalt();
        const voteCommitment = await HiveProver.generateVoteCommitment(a.vote, voteSalt, actionHash);
        const bidCommitment = await HiveProver.generateBidCommitment(a.bid, bidSalt, actionHash);
  ...
  describe('Commitment Verification', () => {
    it('should verify vote commitment matches reveal', async () => {
      ...
      const commitment = await HiveProver.generateVoteCommitment(voteValue, voteSalt, actionHash);
      const revealCommitment = await HiveProver.generateVoteCommitment(voteValue, voteSalt, actionHash);
    ...
    it('should verify bid commitment matches reveal', async () => {
      ...
      const commitment = await HiveProver.generateBidCommitment(bidAmount, bidSalt, actionHash);
      const revealCommitment = await HiveProver.generateBidCommitment(bidAmount, bidSalt, actionHash);
    ...
    it('should reject wrong bid salt on reveal', async () => {
      ...
      const commitment = await HiveProver.generateBidCommitment(bidAmount, originalSalt, actionHash);
      const wrongSaltCommitment = await HiveProver.generateBidCommitment(bidAmount, wrongSalt, actionHash);
