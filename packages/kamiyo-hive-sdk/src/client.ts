...
import idlJson from './idl/hive.json';
import { Idl } from '@coral-xyz/anchor';

// Re-export program ID for convenience
export { SWARMTEAMS_PROGRAM_ID };
...
export class HiveClient {
  ...
  async getAgent(identityCommitment: Uint8Array): Promise<Agent | null> {
    const [agentPDA] = HiveClient.getAgentPDA(identityCommitment);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async getSignal(signalCommitment: Uint8Array): Promise<Signal | null> {
    const [signalPDA] = HiveClient.getSignalPDA(signalCommitment);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async getAggregator(epoch: BN): Promise<SignalAggregator | null> {
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [aggregatorPDA] = HiveClient.getAggregatorPDA(registryPDA, epoch);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async getWithdrawal(agentCommitment: Uint8Array): Promise<WithdrawalRequest | null> {
    const [agentPDA] = HiveClient.getAgentPDA(agentCommitment);
    const [withdrawalPDA] = HiveClient.getWithdrawalPDA(agentPDA);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async getIdentityLink(zkAgent: PublicKey): Promise<IdentityLink | null> {
    const [linkPDA] = HiveClient.getIdentityLinkPDA(zkAgent);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async getCollateralWithdrawal(agentCommitment: Uint8Array): Promise<CollateralWithdrawal | null> {
    const [agentPDA] = HiveClient.getAgentPDA(agentCommitment);
    const [withdrawalPDA] = HiveClient.getCollateralWithdrawalPDA(agentPDA);
    try {
      const accounts = this.program.account as Record<string, { fetch: (key: PublicKey) => Promise<unknown> }>;
  ...
  async registerAgent(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [agentPDA] = HiveClient.getAgentPDA(identityCommitment);
    const [stakeVault] = HiveClient.getStakeVaultPDA(registryPDA);
  ...
  async submitSignal(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [signalPDA] = HiveClient.getSignalPDA(signalCommitment);
    const [nullifierPDA] = HiveClient.getNullifierPDA(nullifier);
    const [treasuryVault] = HiveClient.getTreasuryPDA(registryPDA);
    const kamiyoMint = new PublicKey(KAMIYO_MINT);
  ...
  async voteSwarmAction(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [actionPDA] = HiveClient.getSwarmActionPDA(actionHash);
    const [voteNullifierPDA] = HiveClient.getVoteNullifierPDA(
      actionPDA,
      voteNullifier
    );
    const [voteRecordPDA] = HiveClient.getVoteRecordPDA(
      actionPDA,
      voteNullifier
    );
  ...
  async revealVote(
    ...
    const [actionPDA] = HiveClient.getSwarmActionPDA(actionHash);
    const [voteRecordPDA] = HiveClient.getVoteRecordPDA(actionPDA, voteNullifier);
    ...
    const accounts: Record<string, PublicKey | null> = {
      ...
      identityLink: identityLinkOwner
        ? HiveClient.getIdentityLinkPDA(identityLinkOwner)[0]
        : null,
    };
  ...
  async createSwarmActionBid(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [actionBidPDA] = HiveClient.getSwarmActionBidPDA(actionHash);
  ...
  async voteBidSwarmAction(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [actionBidPDA] = HiveClient.getSwarmActionBidPDA(actionHash);
    const [voteBidNullifierPDA] = HiveClient.getVoteBidNullifierPDA(
      actionBidPDA,
      voteNullifier
    );
    const [voteBidRecordPDA] = HiveClient.getVoteBidRecordPDA(
      actionBidPDA,
      voteNullifier
    );
  ...
  async revealSignal(
    ...
    const [aggregatorPDA] = HiveClient.getAggregatorPDA(registryPDA, registry.epoch);
  ...
  async linkIdentity(
    ...
    const [zkAgentPDA] = HiveClient.getAgentPDA(zkAgentCommitment);
    const [identityLinkPDA] = HiveClient.getIdentityLinkPDA(zkAgentPDA);
  ...
  async depositCollateral(
    ...
    const [registryPDA] = HiveClient.getRegistryPDA();
    const [agentPDA] = HiveClient.getAgentPDA(identityCommitment);
    const [collateralVault] = HiveClient.getCollateralVaultPDA(agentPDA);
  ...
  async slashAgent(
    ...
    const [agentPDA] = HiveClient.getAgentPDA(identityCommitment);
    const [collateralVault] = HiveClient.getCollateralVaultPDA(agentPDA);
    const [treasury] = HiveClient.getTreasuryPDA(registryPDA);
