...
import { MerkleProof, bigintToBytes32 } from '@kamiyo/kamiyo-mitama-merkle';
...
function getCircuitsDir(): string {
  ...
  if (process.env.MITAMA_CIRCUITS_PATH) {
    return process.env.MITAMA_CIRCUITS_PATH;
  }

  // 2. Relative to source file (src/ in development)
  const srcPath = path.resolve(__dirname, '../../../circuits/build/mitama');
  if (fs.existsSync(srcPath)) {
    return srcPath;
  }

  // 3. Relative to dist (dist/ after build)
  const distPath = path.resolve(__dirname, '../../../../circuits/build/mitama');
  if (fs.existsSync(distPath)) {
    return distPath;
  }

  // 4. Relative to workspace root
  const workspacePath = path.resolve(process.cwd(), 'circuits/build/mitama');
  if (fs.existsSync(workspacePath)) {
    return workspacePath;
  }

  // 5. Adjacent to node_modules (installed package)
  const installedPath = path.resolve(__dirname, '../circuits/build/mitama');
  if (fs.existsSync(installedPath)) {
    return installedPath;
  }

  throw new Error(
    'Circuit files not found. Set MITAMA_CIRCUITS_PATH or ensure circuits/build/mitama exists.'
  );
}
