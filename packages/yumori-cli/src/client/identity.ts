...
const YUMORI_DIR = path.join(os.homedir(), '.yumori');
const IDENTITY_FILE = path.join(YUMORI_DIR, 'identity.json');
const SIGNALS_FILE = path.join(YUMORI_DIR, 'signals.json');

export interface YumoriIdentity {
...
function ensureYumoriDir(): void {
  if (!fs.existsSync(YUMORI_DIR)) {
    fs.mkdirSync(YUMORI_DIR, { recursive: true, mode: 0o700 });
  }
}

export function loadIdentity(network: string): YumoriIdentity | null {
  try {
...
export function saveIdentity(identity: YumoriIdentity): void {
  ensureYumoriDir();
...
export function getIdentitySecrets(identity: YumoriIdentity): {
  ownerSecret: bigint;
