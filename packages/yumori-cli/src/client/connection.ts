...
export class SolanaClient {
  ...
  async loadWallet(password?: string): Promise<boolean> {
    ...
    // Try Yumori encrypted wallet
    const yumoriPath = join(homedir(), '.yumori', 'wallet.enc.json');
    if (existsSync(yumoriPath) && password) {
      try {
        const encrypted: EncryptedWallet = JSON.parse(readFileSync(yumoriPath, 'utf8'));
    ...
    const legacyPath = join(homedir(), '.yumori', 'wallet.json');
  ...
  async createWallet(password: string): Promise<Keypair> {
    ...
    const yumoriDir = join(homedir(), '.yumori');
    if (!existsSync(yumoriDir)) {
      mkdirSync(yumoriDir, { recursive: true, mode: 0o700 });
    }

    // Encrypt and save wallet
    const encrypted = encryptSecretKey(this.keypair.secretKey, password);
    const walletPath = join(yumoriDir, 'wallet.enc.json');
  ...
  async migrateUnencryptedWallet(password: string): Promise<boolean> {
    ...
    try {
      ...
      const newPath = join(homedir(), '.yumori', 'wallet.enc.json');
